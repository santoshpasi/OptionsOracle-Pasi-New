...
namespace OptionsOracle.Data
{
    ...
    {
        partial class OptionsTableDataTable
        {
        }

        partial class QuotesTableDataTable
        {
        }
        ...
        public void Load(string filename)
        {
            ...
            foreach (OptionsTableRow row in OptionsTable)
            {
                ...
                if (row.IsChngOINull()) row.ChngOI = 0;
                if (row.IsOIAnalysisNull()) row.OIAnalysis = "N/A";
            }
        ...
        public void UpdateDerivedData(Quote quote, bool calc_imp_vol, bool calc_itm_prob, bool calc_stddev, bool calc_indicators)
        {
            ...
            try
            {
                ...
                if (calc_imp_vol)
                {
                    ...
                    //MessageBox.Show("Quote Futures 1 : " + quote.price.futurenearlast);
                    UpdateStockImpliedVolatility(quote, option_list);
                    //MessageBox.Show("Quote Futures 2 : " + quote.price.futurenearlast);
                ...
                // MessageBox.Show("Quote Futures 4 : " + quote.price.futurenearlast);
        ...
        public ArrayList GetExperiationStrikeListWithCommisionFactor()
        {
            ...
            for (int i = 0; i < PositionsTable.Rows.Count; i++)
            {
                try
                {
                    ...
                    if (!list.Contains((double)row["Strike"]) && (!((string)row["Type"]).Contains("Stock") || !((string)row["Type"]).Contains("Future")))
                    {
        ...
        public ArrayList GetStockPriceList()
        {
            ...
            for (int i = 0; i < PositionsTable.Rows.Count; i++)
            {
                try
                {
                    ...
                    if (!list.Contains((double)row["Price"]) && (((string)row["Type"]).Contains("Stock") || ((string)row["Type"]).Contains("Future")))
        ...
        public Option GetOptionFromOptionTableRow(OptionsTableRow row)
        {
            ...
            try
            {
                ...
                option.change_oi = row.IsOpenIntNull() ? 0 : row.ChngOI;
                option.oi_analysis = row.IsOpenIntNull() ? "N/A" : row.OIAnalysis;
        ...
        public double FutureNearLastPrice
        {
            get { QuotesTableRow row = GetQuoteRow(); if (row.IsLastNull()) return double.NaN; else return row.FutureNearLast; }
        ...
        public double FutureLotSize
        {
            get { QuotesTableRow row = GetQuoteRow(); if (row.IsLastNull()) return double.NaN; else return row.FutureLotSize; }
        ...
        private void AddOptionEntry(Quote quote, Option option)
        {
            ...
            try
            {
                ...
                //row.Expiration = DateTime.Parse(option.expiration.ToLongDateString());
                ...
                row.ChngOI = option.change_oi;
                row.OIAnalysis = option.oi_analysis;
                ...
                if (option.type == "Call")
                {
                    ...
                    if (option.strike > quote.price.last * 1.10 && quote.price.last >= 1000) row.TheMoney = "OTM";
                    else if (option.strike > quote.price.last * 1.20 && quote.price.last < 1000) row.TheMoney = "OTM";
                    else if (option.strike < quote.price.last * 0.95 && quote.price.last >= 1000) row.TheMoney = "ITM";
                ...
                else
                {
                    ...
                    if      (option.strike > quote.price.last * 1.05 && quote.price.last >= 1000) row.TheMoney = "ITM";
                    else if (option.strike > quote.price.last * 1.10 && quote.price.last < 1000) row.TheMoney = "ITM";
                    else if (option.strike < quote.price.last * 0.90 && quote.price.last >= 1000) row.TheMoney = "OTM";
                    else if (option.strike < quote.price.last * 0.80 && quote.price.last < 1000) row.TheMoney = "OTM";
                }
        ...
        private void AddQuoteEntry(Quote quote)
        {
            ...
            try
            {
                ...
                row.FutureNearLast = quote.price.futurenearlast;
                row.FutureLotSize = quote.price.futurelot;
        ...
        private void UpdateStockImpliedVolatility(Quote quote, ArrayList option_list)
        {
            ...
            //MessageBox.Show("Quote Futures 10 : " + quote.price.futurenearlast);
        ...
        public Quote GetQuoteFromQuoteTableRow(QuotesTableRow row)
        {
            ...
            try
            {
                ...
                if (row.IsFutureNearLastNull()) quote.price.futurenearlast = double.NaN;
                else quote.price.futurenearlast = row.FutureNearLast;

                if (row.IsFutureLotSizeNull()) quote.price.futurelot = double.NaN;
                else quote.price.futurelot = row.FutureLotSize;
